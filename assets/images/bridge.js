...
(function(){
	...
	var Bridge0 = 
	{
		...
		runFileDialog: function(caption, dir, filter, callback){

			// QString response - path to file
			_Bridge_.openFileDialog(caption,dir,filter,function(response){
		...
		runImgFileDialog: function(caption, dir, options, callback){
			...
			Bridge.runFileDialog(caption,dir,"Images (*.jpg *.jpeg *.png *.gif *.bmp *.tiff)",function(path){
				...
				{
					Bridge.resizeImage(path, options.width, options.height, options.keepAspectRatio, options.quality, options.format, callback);
				}
				else
					if (callback) callback(path);
